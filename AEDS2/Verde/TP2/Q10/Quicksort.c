...
// Função recursiva para realizar o quicksort em um array de Jogador com base em seu estadoNascimento e nome.
void quicksortRec(Jogador* jogadores, int esq, int dir, int* comparacoes, int* movimentacoes) {
    int i = esq, j = dir;
    Jogador pivo = jogadores[(dir + esq)/2];
    while(i <= j) {
        
        while(((*comparacoes)++ ,strcmp(jogadores[i].estadoNascimento, pivo.estadoNascimento) < 0) || 
        ((*comparacoes)++ ,strcmp(jogadores[i].estadoNascimento, pivo.estadoNascimento) == 0 && strcmp(jogadores[i].nome, pivo.nome) < 0) ) {
            i++;
        }
        
        while(((*comparacoes)++ ,strcmp(jogadores[j].estadoNascimento, pivo.estadoNascimento) > 0) || 
        ((*comparacoes)++ ,strcmp(jogadores[j].estadoNascimento, pivo.estadoNascimento) == 0 && strcmp(jogadores[j].nome, pivo.nome) > 0) ) {
            j--;
        }
        if(i <= j) {
            (*movimentacoes) += 2;
...
void quicksort(Jogador * jogadores, int numJogadores, int *comparacoes, int *movimentacoes) {
    quicksortRec(jogadores, 0, numJogadores-1, comparacoes, movimentacoes);
}
...
int main() {
    ...
    quicksort(vetorJogadores, contadorVetor, &comparacoes, &movimentacoes);
    ...
    FILE *logFile = fopen("800854_quicksort.txt", "w");
    if (logFile != NULL) {
